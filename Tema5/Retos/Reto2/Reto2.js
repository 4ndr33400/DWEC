...
function transitionText() {
    // Seleccionamos todos los párrafos dentro del contenedor
    let paragraphs = document.querySelectorAll('.container p');
    let i = 0;

    // Función interna para animar párrafo por párrafo
    function animationParagraph() {
        if (i < paragraphs.length) {
            // Aplicamos estilos al párrafo actual
            paragraphs[i].style.opacity = 1;
            paragraphs[i].style.animation = "transitionParagraph 2s ease-in-out";

            // Pasamos al siguiente párrafo después de 2 segundos
            i++;
            setTimeout(animationParagraph, 2000);
        }
    }

    // Llamamos a la función inicial para comenzar la animación
    animationParagraph();
}
